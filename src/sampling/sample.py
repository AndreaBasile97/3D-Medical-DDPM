...
for k, inputfile in enumerate(mask_list):
    ...
    for i, bsize in enumerate(batches):
        ...
        all_images_list = list(
            map(
                lambda n: diffusion.sample(
                    batch_size=n, condition_tensors=condition_tensors
                ),
                [bsize],
            )
        )
        ...
        for b, c in enumerate(counted_samples):
            ...
            # All'interno del tuo ciclo, prima di salvare i file
            img_path = os.path.join(img_dir, f"{counter}_{msk_name}")
            msk_path = os.path.join(msk_dir, f"{counter}_{msk_name}")
