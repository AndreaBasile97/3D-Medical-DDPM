...
git_configuration := "Do you want to configure git credentials? [yes|no]"
github_repo_init := "Do you want to initialize the GitHub repository? [yes|no]"
repository_url := "Enter the URL of the GitHub repository"
author_name := "Andrea Basile"
author_email :="andreabasile97@gmail.com"
data_dir := "Enter the path for the dataset to move into the 'data/raw' directory, otherwise enter 'no'."
...
initialize_repo:
    ...
    ifeq ($(github_repo_init),"yes")
	    git remote add origin Enter the URL of the GitHub repository
		git add .
	    git commit -m 'initializing from cookiecutter'
	    git branch -M main
	    git push -u origin main
        ifneq ($(data_dir),"no")
	        mv $(data_dir) 3D-Medical-DDPM/data/raw
        else 
